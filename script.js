...
function onJudge(shouldSave) {
  ...
  function updateExitSelect() {
    ...
    // Placeholder (so mobile/desktop both start with "no selection")
    const ph = document.createElement("option");
    ph.value = "";
    ph.textContent = "（トレードを選択）";
    ph.disabled = true;
    ph.selected = true;
    elExitSelect.appendChild(ph);
...
`;
  ...
  function saveExit() {
    ...
    // --- reset UI to the same as "reload" after save ---
    selectedExitId = null;
    if (elExitSelect) elExitSelect.value = "";
    ...
    // Make sure badge/button are reset
    updateExitEditUI();
  }
