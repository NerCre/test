...
function updateExitEditUI() {
  ...
  function isSameFeature(r, current) {
    ...
    // - Only compare fields the user actually filled
    // - Give more weight to "決め手インジ/サイン" and market-context fields
    const weights = {
      // market context
      trend_5_20_40: 1.4,
      price_vs_ema200: 1.2,
      ...
      // execution style
      tradeMethod: 1.0,
      // plan (重要：TP/LSの違いで勝率が割れるので、ここを無視すると“混ぜ物”になる)
      tpType: 1.1,
      lsType: 1.1,
      // decisive (最重要)
      decisiveIndicator: 1.6,
      decisiveSignal: 1.6
    };
  ...
  function judge(current) {
    ...
    // 2) nearest neighbors by similarity
    const withScore = candidates
      .map((r) => ({ r, score: isSameFeature(r, current) }))
      .sort((a, b) => b.score - a.score);

    // Use top neighbors with a soft minimum similarity.
    // Too strict => data starvation. Too loose => garbage in.
    const MIN_SIM = 0.55;
    const MAX_NEIGHBORS = 60;
    const neighbors = withScore.filter((x) => x.score >= MIN_SIM).slice(0, MAX_NEIGHBORS);
    ...
    // weight = similarity^alpha (alpha>1 to emphasize closer matches)
    const alpha = 3;

    function effSampleSize(ws) {
    ...
    function calcDirStats(dir) {
      const group = neighbors.filter((p) => (p.r.directionTaken || p.r.directionPlanned) === dir);

      if (!group.length) {
        return { dir, n: 0, ess: 0, winRate: null, avgProfit: null, avgLoss: null, expectedMove: null, ev: null, simAvg: 0 };
      }

      const ws = group.map((g) => Math.pow(g.score, alpha));
      const ess = effSampleSize(ws);

      // Win-rate (weighted)
      let wTotal = 0;
      let wWin = 0;

      // EV (expected profit per trade) as weighted mean of profit
      const profitPairs = [];
      ...
      // Expected move (favorable excursion)
      const movePairs = [];
      ...
      for (let i = 0; i < group.length; i++) {
        ...
        if (Number.isFinite(pr)) {
          profitPairs.push({ w, x: pr });
          if (pr > 0) { wWin += w; winPairs.push({ w, x: pr }); }
          if (pr < 0) { lossPairs.push({ w, x: pr }); }
        }

        if (dir === "long") {
          if (Number.isFinite(r.highDuringTrade) && Number.isFinite(r.entryPrice)) {
            const mv = Math.max(0, r.highDuringTrade - r.entryPrice);
            movePairs.push({ w, x: mv });
          }
        } else if (dir === "short") {
          if (Number.isFinite(r.lowDuringTrade) && Number.isFinite(r.entryPrice)) {
            const mv = Math.max(0, r.entryPrice - r.lowDuringTrade);
            movePairs.push({ w, x: mv });
          }
        }
      }

      const winRate = wTotal > 0 ? (wWin / wTotal) * 100 : null;
      const ev = weightedMean(profitPairs); // ← 正しい期待値（勝率を自動で反映）
      const avgProfit = weightedMean(winPairs);
      const avgLoss = weightedMean(lossPairs);
      const expectedMove = weightedMean(movePairs);

      // similarity quality (weighted by same weights)
      const simAvg = wTotal > 0 ? group.reduce((s, g, i) => s + ws[i] * g.score, 0) / wTotal : 0;

      return { dir, n: group.length, ess, winRate, avgProfit, avgLoss, expectedMove, ev, simAvg };
    }

    const longS = calcDirStats("long");
    const shortS = calcDirStats("short");

    // Minimum data gates (avoid “たまたま当たった”推薦)
    const MIN_CASES = 6;
    const MIN_ESS = 3;

    function isUsable(s) {
      return s.n >= MIN_CASES && s.ess >= MIN_ESS && s.winRate != null && s.ev != null;
    }

    const usable = [longS, shortS].filter(isUsable);

    let recommendation = "flat";
    let chosen = null;

    if (usable.length) {
      // pick by EV (expected profit), tie-break by winRate then ESS
      usable.sort((a, b) => {
        ...
        return (b.ess ?? 0) - (a.ess ?? 0);
      });
      chosen = usable[0];

      // Additional gates: must be positive EV, and exceed minWinRate
      const EV_MIN = 0;     // 期待値マイナスは論外
      const EV_GAP = 10;    // もう一方との差が小さいときはノーポジ（迷う局面）
      const other = chosen.dir === "long" ? shortS : longS;

      const gap = (chosen.ev ?? 0) - (other.ev ?? -Infinity);
      if ((chosen.ev ?? -Infinity) > EV_MIN && (chosen.winRate ?? 0) >= minWinRate && gap >= EV_GAP) {
        recommendation = chosen.dir;
      } else {
        recommendation = "flat";
      }
    }

    // pseudo cases = used neighbor count (after similarity cut)
    const pseudoCaseCount = neighbors.length;

    // confidence: blend winRate, sample size (ESS), similarity quality, and edge size
    let confidence = 0;
    let winRate = null;

    if (recommendation !== "flat" && chosen) {
      winRate = chosen.winRate;

      const wrN = clamp((chosen.winRate ?? 0) / 100, 0, 1);
      const sampleN = clamp(Math.log10((chosen.ess ?? 0) + 1) / 1.1, 0, 1);   // ESS=10で≈0.9
      const simN = clamp(chosen.simAvg ?? 0, 0, 1);

      const risk = Math.max(1, Math.abs(chosen.avgLoss ?? 0));
      const edgeN = clamp((chosen.ev ?? 0) / (risk * 0.6), 0, 1);

      confidence = clamp((wrN * 0.45 + sampleN * 0.25 + simN * 0.20 + edgeN * 0.10) * 100, 0, 100);
    } else {
      // flatでも「データはあるが微妙」な局面は0より上にして、判断材料にする
      const avgSimAll = neighbors.length ? neighbors.reduce((s, x) => s + x.score, 0) / neighbors.length : 0;
      const simN = clamp(avgSimAll, 0, 1);
      const nN = clamp(Math.log10(neighbors.length + 1) / 1.3, 0, 1);
      confidence = clamp((simN * 0.6 + nN * 0.4) * 35, 0, 35); // flat時は最大35%
    }

    // output (compatible with existing UI)
    if (recommendation === "long") chosen = longS;
    if (recommendation === "short") chosen = shortS;
    ...
    return {
      ...
      expectedMoveUnit: "円",
      confidence,
      winRate: (recommendation === "flat") ? null : (chosen.winRate ?? null),
