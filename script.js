(() => {
  ...
  const MASTER_DEFAULTS = Object.freeze({
    ...
    evMin: 0.10,             // EV_MIN (R) 例: 0.10R
    evGap: 0.10,             // EV_GAP (R) 例: 0.10R
    rrGateEnabled: true,     // RR gate strict（RRが下限未満なら原則ノーポジ）
    rrMin: 1.0,              // RR_MIN
    rrAllowMissing: true,    // TP/SL未入力（RR不明）はゲート対象外にする（ただし警告）
  ...
  function initMasterTab() {
    ...
    const elEvMin = $("#master-evMin");
    const elEvGap = $("#master-evGap");
    ...
    elEvMin.value = String(s.evMin ?? MASTER_DEFAULTS.evMin);
    elEvGap.value = String(s.evGap ?? MASTER_DEFAULTS.evGap);
    elRrMin.value = String(s.rrMin ?? MASTER_DEFAULTS.rrMin);
    elRrGate.checked = !!s.rrGateEnabled;
    elRrAllowMissing.checked = !!s.rrAllowMissing;
    ...
    $("#btn-master-save")?.addEventListener("click", () => {
      ...
      const next = {
        ...
        evMin: Number(elEvMin.value) || 0,
        evGap: Math.max(0, Number(elEvGap.value) || 0),
    ...
    $("#btn-master-reset")?.addEventListener("click", () => {
      ...
      elEvMin.value = String(d.evMin);
      elEvGap.value = String(d.evGap);
...
out.entryPrice = out.entryPrice ?? null;
    ...
    // Backfill for old records (size/SLが入っている場合のみ)
    if (out.hasResult && (out.profitPerUnit == null) && typeof out.profit === "number" && Number.isFinite(out.profit) && typeof out.size === "number" && Number.isFinite(out.size) && out.size !== 0) {
      out.profitPerUnit = out.profit / out.size;
    }
    if (out.hasResult && (out.riskPerUnit == null)) {
      const rp = computeRiskPerUnit(out.symbol, Number(out.entryPrice), Number(out.cutLossPrice), Number(out.feePerUnit));
      if (rp != null) out.riskPerUnit = rp;
    }
    if (out.hasResult && (out.rMultiple == null)) {
      const rm = computeRMultiple(Number(out.profitPerUnit), Number(out.riskPerUnit));
      if (rm != null) out.rMultiple = rm;
    }
...
function initTabs() {
  ...
  // ===== Setup (決め手インジ × 決め手シグナル) =====
  function getSetupId(r) {
    const ind = (r && typeof r.decisiveIndicator === "string") ? r.decisiveIndicator.trim() : "";
    const sig = (r && typeof r.decisiveSignal === "string") ? r.decisiveSignal.trim() : "";
    if (!ind || !sig) return "";
    return `${ind}::${sig}`;
  }

  function getSetupLabel(r) {
    const ind = (r && typeof r.decisiveIndicator === "string") ? r.decisiveIndicator.trim() : "";
    const sigText = (r && typeof r.decisiveSignalText === "string" && r.decisiveSignalText.trim()) ? r.decisiveSignalText.trim() : "";
    const sig = (r && typeof r.decisiveSignal === "string") ? r.decisiveSignal.trim() : "";
    const indLabel = ind ? indicatorLabel(ind) : "—";
    const sigLabel = sigText || sig || "—";
    return `${indLabel} × ${sigLabel}`;
  }
  ...
  // rows: {rMultiple, datetimeExit/datetimeEntry}
  function computeRStatsFromRows(rows) {
    const list = (rows || []).filter((r) => r && typeof r.rMultiple === "number" && Number.isFinite(r.rMultiple));
    const n = list.length;
    if (!n) return { n: 0, winRate: null, avgR: null, medianR: null, pf: null, maxDD: null, evR: null };

    const wins = list.filter((r) => r.rMultiple > 0);
    const winRate = (wins.length / n) * 100;

    const rs = list.map((r) => r.rMultiple);
    const avgR = rs.reduce((a, b) => a + b, 0) / n;
    const medianR = medianOf(rs);

    const sumPos = rs.filter((x) => x > 0).reduce((a, b) => a + b, 0);
    const sumNeg = rs.filter((x) => x < 0).reduce((a, b) => a + b, 0);
    const pf = (sumNeg < 0) ? (sumPos / Math.abs(sumNeg)) : (sumPos > 0 ? Infinity : null);

    // max drawdown (R) - simple: cumulative R by exit datetime
    const sorted = list.slice().sort((a, b) => {
      const ta = Date.parse(a.datetimeExit || a.datetimeEntry || "") || 0;
      const tb = Date.parse(b.datetimeExit || b.datetimeEntry || "") || 0;
      return ta - tb;
    });
  ...
  function buildSetupStatsMap(list) {
    const map = new Map();
    (list || []).forEach((r) => {
      const id = getSetupId(r);
      if (!id) return;
      if (!map.has(id)) map.set(id, { label: getSetupLabel(r), rows: [] });
...
function updateExitEditUI() {
  ...
  function judge(current) {
    ...
    // Stage B: symbol/timeframe + realized result
    ...
    const EV_MIN = Number.isFinite(ms.evMin) ? ms.evMin : MASTER_DEFAULTS.evMin;
    const EV_GAP = Number.isFinite(ms.evGap) ? ms.evGap : MASTER_DEFAULTS.evGap;

    debug.params = { TH, MAX_NEIGHBORS, MIN_CASES, MIN_ESS, alpha, EV_MIN, EV_GAP, minComparableFeatures: (Number.isFinite(ms.minComparableFeatures) ? ms.minComparableFeatures : MASTER_DEFAULTS.minComparableFeatures) };
...
// group by directionTaken (STRICT). Planned direction is not used for evaluation.
    ...
    debug.stages.afterWeighting = pseudo.length;
    debug.ess = ess;

    // ===== Setup prior (決め手インジ × 決め手シグナル) =====
    const setupId = getSetupId(current);
    const setupLabel = setupId ? getSetupLabel(current) : "";
    const setupPool = setupId ? candidates.filter((r) => getSetupId(r) === setupId) : [];
    const setupAll = setupId ? computeRStatsFromRows(setupPool) : null;
    const setupLong = setupId ? computeRStatsFromRows(setupPool.filter((r) => r.directionTaken === "long")) : null;
    const setupShort = setupId ? computeRStatsFromRows(setupPool.filter((r) => r.directionTaken === "short")) : null;

    // Shrinkage pivot (推奨: 30)
    const N0_SETUP = 30;

    const calcESS = (weights) => {
      const ws = (weights || []).map((x) => (typeof x === "number" && Number.isFinite(x)) ? x : 0);
    ...
    for (const d of dirs) {
      if (d === "flat") {
        statsByDir[d] = { dir: d, n: pseudo.length, essDir: ess, winRate: null, avgProfit: null, avgLoss: null, evKnn: null, evSetup: null, ev: null, expectedMove: null, setupN: 0, setupW: null };
        continue;
      }

      const rows = weightedRows.filter((r) => r.directionTaken === d);
      const wSum = rows.reduce((a, r) => a + (r._w || 0), 0);
      const essDir = calcESS(rows.map((r) => r._w || 0));

      const winsW = rows.filter((r) => (typeof r.rMultiple === "number" && Number.isFinite(r.rMultiple) && r.rMultiple > 0))
        .reduce((a, r) => a + (r._w || 0), 0);

      const winRate = (wSum > 0) ? (winsW / wSum) * 100 : null;

      // Weighted means: R (learning metric)
      const evKnn = (wSum > 0) ? (rows.reduce((a, r) => a + ((r.rMultiple || 0) * (r._w || 0)), 0) / wSum) : null;

      // For display only: per-unit yen averages
      const avgProfit = (() => {
        const rw = rows.filter((r) => typeof r.profitPerUnit === "number" && Number.isFinite(r.profitPerUnit) && r.profitPerUnit > 0);
        const w = rw.reduce((a, r) => a + (r._w || 0), 0);
        return (w > 0) ? (rw.reduce((a, r) => a + (r.profitPerUnit * (r._w || 0)), 0) / w) : null;
      })();
      ...
      // Setup EV (direction-specific)
      const setupStats = (d === "long") ? setupLong : setupShort;
      const setupN = setupStats?.n ?? 0;
      const setupEV = (setupN > 0) ? (setupStats?.evR ?? null) : null;

      // Only start affecting Judge from N>=30 (安全策)
      const useSetup = (setupN >= 30) && (typeof setupEV === "number") && Number.isFinite(setupEV) && (typeof evKnn === "number") && Number.isFinite(evKnn);

      const wMix = useSetup ? (N0_SETUP / (N0_SETUP + setupN)) : null;
      const evFinal = useSetup ? ((wMix * evKnn) + ((1 - wMix) * setupEV)) : evKnn;
      ...
      statsByDir[d] = {
        ...
        avgLoss,
        evKnn,
        evSetup: useSetup ? setupEV : null,
        ev: evFinal,
        expectedMove: evFinal,     // unit: R
        setupN,
        setupW: useSetup ? wMix : null,
        setupActive: useSetup
      };
    }

    // ===== Choose direction =====
    const longS = statsByDir.long;
    const shortS = statsByDir.short;

    const choices = [];
    if (longS && longS.n >= MIN_CASES && (longS.essDir ?? 0) >= MIN_ESS && (longS.winRate ?? 0) >= minWinRate) choices.push("long");
    if (shortS && shortS.n >= MIN_CASES && (shortS.essDir ?? 0) >= MIN_ESS && (shortS.winRate ?? 0) >= minWinRate) choices.push("short");

    let candidate = "flat";
    if (choices.length) {
      candidate = choices.sort((a, b) => {
        const A = statsByDir[a];
        const B = statsByDir[b];
    ...
    // Expected value gate (R)
    if (recommendation !== "flat") {
      if ((chosen.ev ?? -Infinity) < EV_MIN) {
        reason = `期待値が低い（EV=${fmtNum(chosen.ev, 3)}R < EV_MIN=${fmtNum(EV_MIN, 3)}R）。やるな。`;
        recommendation = "flat";
      }
    }

    // EV gap gate (R) - long/short差が小さいなら見送り
    if (recommendation !== "flat") {
      const evL = longS?.ev;
      const evS = shortS?.ev;
      if (typeof evL === "number" && typeof evS === "number" && Number.isFinite(evL) && Number.isFinite(evS)) {
        if (Math.abs(evL - evS) < EV_GAP) {
          reason = `差が小さい（|EV_long−EV_short|=${fmtNum(Math.abs(evL - evS), 3)}R < EV_GAP=${fmtNum(EV_GAP, 3)}R）。無理に選ぶな。`;
          recommendation = "flat";
        }
      }
    }

    // Confidence (Wilson lower bound using effective sample)
    const nEff = Math.max(1, chosen.essDir ?? chosen.n ?? 1);
    const p = clamp((winRate ?? 0) / 100, 0, 1);
    const z = 1.96;
    const z2 = z * z;
    ...
    const currentRiskPerUnit = computeRiskPerUnit(current.symbol, current.entryPrice, current.cutLossPrice, current.feePerUnit);
    const expectedR = (recommendation === "flat") ? null : (chosen.expectedMove ?? null);
    const expectedYenPerUnit = (typeof expectedR === "number" && Number.isFinite(expectedR) && typeof currentRiskPerUnit === "number" && Number.isFinite(currentRiskPerUnit))
      ? (expectedR * currentRiskPerUnit)
      : null;

    const expectedYenTotal = (typeof expectedYenPerUnit === "number" && Number.isFinite(expectedYenPerUnit) && typeof current.size === "number" && Number.isFinite(current.size))
      ? (expectedYenPerUnit * current.size)
      : null;

    return {
      recommendation,
      expectedMove: expectedR,
      expectedMoveUnit: "R",
      expectedYenPerUnit,
      expectedYenTotal,
      currentRiskPerUnit,
      confidence: (recommendation === "flat") ? 0 : confidence,
      winRate: (recommendation === "flat") ? null : (winRate ?? null),
      avgProfit: (recommendation === "flat") ? null : (chosen.avgProfit ?? null),  // per unit yen
      avgLoss: (recommendation === "flat") ? null : (chosen.avgLoss ?? null),      // per unit yen
      reason,
      pseudoCaseCount,
      ess,
      ...
      evBreakdown: {
        long: { evKnn: longS?.evKnn ?? null, evSetup: longS?.evSetup ?? null, evFinal: longS?.ev ?? null, setupN: longS?.setupN ?? 0, setupW: longS?.setupW ?? null, setupActive: longS?.setupActive ?? false },
        short: { evKnn: shortS?.evKnn ?? null, evSetup: shortS?.evSetup ?? null, evFinal: shortS?.ev ?? null, setupN: shortS?.setupN ?? 0, setupW: shortS?.setupW ?? null, setupActive: shortS?.setupActive ?? false }
      },
      debug
    };
    return {
...
`;
  ...
  function computeProfitPerUnit(symbol, direction, entryPrice, exitPrice, feePerUnit) {
    ...
    const profitPerUnit = (diff * mult) - feePerUnit;
    return Number.isFinite(profitPerUnit) ? profitPerUnit : null;
  }

  // 1枚あたりリスク（円）: 想定SLまでの距離 + 手数料（往復想定）
  function computeRiskPerUnit(symbol, entryPrice, cutLossPrice, feePerUnit) {
    if (![entryPrice, cutLossPrice, feePerUnit].every((n) => typeof n === "number" && Number.isFinite(n))) return null;
    const mult = symbolMultiplier(symbol);
    const riskMove = Math.abs(cutLossPrice - entryPrice);
    const riskPerUnit = (riskMove * mult) + feePerUnit;
    return (riskPerUnit > 0 && Number.isFinite(riskPerUnit)) ? riskPerUnit : null;
  }

  // R倍（= 1枚あたり損益 / 1枚あたりリスク）
  function computeRMultiple(profitPerUnit, riskPerUnit) {
    if (!(typeof profitPerUnit === "number" && Number.isFinite(profitPerUnit))) return null;
    if (!(typeof riskPerUnit === "number" && Number.isFinite(riskPerUnit)) || riskPerUnit <= 0) return null;
    const r = profitPerUnit / riskPerUnit;
  ...
  function saveExit() {
    ...
    const profitPerUnit = computeProfitPerUnit(
      r.symbol,
      r.directionTaken || r.directionPlanned,
      r.entryPrice,
      exitPrice,
    ...
    const riskPerUnit = computeRiskPerUnit(
      r.symbol,
      r.entryPrice,
      r.cutLossPrice,
      r.feePerUnit
    ...
    const rMultiple = computeRMultiple(profitPerUnit, riskPerUnit);
...
function populateIndicatorStateFilterOptions(indicatorKey) {
  ...
  function renderSetupStatsTable(list) {
    const box = document.getElementById("stats-setup-table");
    if (!box) return;

    const usable = (list || []).filter((r) => r && r.hasResult && typeof r.rMultiple === "number" && Number.isFinite(r.rMultiple) && getSetupId(r));
    if (!usable.length) {
      box.innerHTML = '<div class="muted">（表示できるデータがありません。決め手インジと決め手シグナル、損益、SLが入っているレコードが必要です）</div>';
      return;
    }

    const map = buildSetupStatsMap(usable);
    const rows = [];
    map.forEach((v, id) => {
      const s = computeRStatsFromRows(v.rows);
      rows.push({
        id,
        label: v.label,
        ...s
    ...
    // show from N>=10
    const rows10 = rows.filter((r) => (r.n ?? 0) >= 10).sort((a, b) => (b.n ?? 0) - (a.n ?? 0));
    if (!rows10.length) {
      box.innerHTML = '<div class="muted">（セットアップ別の表示は N≥10 からです）</div>';
      return;
    }
    ...
    const t = `
      <table class="stats-table">
        ...
        <tbody>
          ${rows10.map((r) => `
            <tr>
              <td title="${esc(r.id)}">${esc(r.label)}</td>
              <td class="num">${r.n}</td>
              <td class="num">${r.winRate == null ? "—" : `${fmtNum(r.winRate, 1)}%`}</td>
              <td class="num">${r.avgR == null ? "—" : `${fmtNum(r.avgR, 3)}R`}</td>
              <td class="num">${r.medianR == null ? "—" : `${fmtNum(r.medianR, 3)}R`}</td>
              <td class="num">${r.pf == null ? "—" : (r.pf === Infinity ? "∞" : fmtNum(r.pf, 2))}</td>
              <td class="num">${r.maxDD == null ? "—" : `${fmtNum(r.maxDD, 3)}R`}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
...
function safeRenderStats() {
  ...
  function renderStats() {
    ...
    renderSetupStatsTable(filtered);
  ...
  function renderJudge(result, symbol, timeframe) {
    ...
    const expected = (() => {
    if (isBelow || result.expectedMove == null || !Number.isFinite(result.expectedMove)) return "—";
    const r = result.expectedMove;
    const rStr = `${r >= 0 ? "+" : ""}${fmtNum(r, 3)}R`;
    const y1 = (typeof result.expectedYenPerUnit === "number" && Number.isFinite(result.expectedYenPerUnit)) ? formatYen(result.expectedYenPerUnit) : null;
    const yT = (typeof result.expectedYenTotal === "number" && Number.isFinite(result.expectedYenTotal)) ? formatYen(result.expectedYenTotal) : null;

    if (y1 && yT) return `${rStr}（約${y1}/枚, ${yT}合計）`;
    if (y1) return `${rStr}（約${y1}/枚）`;
    return rStr;
  })();
    ...
    elJudgeOutput.innerHTML = `
      <div class="judge-grid">
        ...
        <div><strong>推定値幅</strong><div>${expected}</div></div>
        <div><strong>平均利益</strong><div>${result.avgProfit == null ? "—" : `${formatYen(result.avgProfit)}/枚`}</div></div>
        <div><strong>平均損失</strong><div>${result.avgLoss == null ? "—" : `${formatYen(result.avgLoss)}/枚`}</div></div>

      <div class="full">
        <details class="details">
          <summary><strong>セットアップ統計（決め手インジ×シグナル）</strong></summary>
          <div class="setup-box">
            <div><strong>今回</strong>：${(result.setup && result.setup.label) ? result.setup.label : "—"}</div>
            <div class="small muted">※ Judgeへの反映は「方向別N≥30」から。未満は表示のみ。</div>
            <div class="small">
              全体：N=${result.setup?.all?.n ?? 0} / 勝率=${result.setup?.all?.winRate==null? "—" : `${fmtNum(result.setup.all.winRate,1)}%`} / 平均R=${result.setup?.all?.avgR==null? "—" : `${fmtNum(result.setup.all.avgR,3)}R`} / PF=${result.setup?.all?.pf==null? "—" : (result.setup.all.pf===Infinity? "∞" : fmtNum(result.setup.all.pf,2))}
            </div>
            <div class="small">
              Long：N=${result.setup?.long?.n ?? 0} / EV=${result.evBreakdown?.long?.evSetup==null? "—" : `${fmtNum(result.evBreakdown.long.evSetup,3)}R`} / w=${result.evBreakdown?.long?.setupW==null? "—" : fmtNum(result.evBreakdown.long.setupW,2)}
            </div>
            <div class="small">
              Short：N=${result.setup?.short?.n ?? 0} / EV=${result.evBreakdown?.short?.evSetup==null? "—" : `${fmtNum(result.evBreakdown.short.evSetup,3)}R`} / w=${result.evBreakdown?.short?.setupW==null? "—" : fmtNum(result.evBreakdown.short.setupW,2)}
            </div>
          </div>
        </details>
      </div>
