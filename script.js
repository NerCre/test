...
function onJudge(shouldSave) {
  ...
  if (!validateRequiredSelects()) return false;
...
`;
  ...
  function clearExitFormOnly() {
    ...
    // クリア操作はエラーではないため、メッセージ欄は空にしてトーストで通知する
    clearMsg();
  }
...
function escapeHtml(s) {
  ...
  function bind() {
    $("#btn-judge")?.addEventListener("click", () => {
      const ok = onJudge(false);
      if (ok) showToast("判定しました", "success");
    });
    $("#btn-save-entry")?.addEventListener("click", () => onJudge(true));
    $("#btn-clear-entry")?.addEventListener("click", () => {
      clearEntryForm();
      showToast("エントリーをクリアしました", "success");
    });
...
$("#btn-exit-clear")?.addEventListener("click", () => {
      clearExitSelectionAndForm();
