(() => {
  ...
  const MASTER_DEFAULTS = Object.freeze({
    ...
    rrMin: 1.0,              // RR_MIN
    rrAllowMissing: false,    // allow when TP/SL missing
...
function updateExitEditUI() {
  ...
  function isSameFeature(r, current) {
    // Similarity is deliberately punitive for missing values.
    // Missing in either side is treated as "unknown = risky" (i.e., mismatch).
    const keys = [
      "waveCount",
      "dowShape",
      "trend_5_20_40",
    ...
    // Categorical features: always count (missing => mismatch)
    for (const k of keys) {
      denom += 1;
      const a = toKey(current[k]);
      const b = toKey(r[k]);
      if (!isBlank(a) && !isBlank(b) && a === b) match += 1;
    }
    ...
    denom += wRR;
    match += wRR * relSim(curP.rr, recP.rr);

    denom += wTPD;
    match += wTPD * relSim(curP.tpDist, recP.tpDist);

    denom += wSLD;
    match += wSLD * relSim(curP.slDist, recP.slDist);
  ...
  function judge(current) {
    ...
    const candidates = records.filter((r) => {
      ...
      if (!allowTypes.includes(tt)) return false;
      if (!allowStatuses.includes(cs)) return false;
...
// group by directionTaken (STRICT). Planned direction is not used for evaluation.
    ...
    for (const d of dirs) {
      ...
      // Weighted win prob
      const wWins = rows.reduce((s, x) => s + ((x.r.profit > 0) ? x.w : 0), 0);
      ...
      const avgProfit = (wWins > 0)
        ? rows.reduce((s, x) => s + ((x.r.profit > 0) ? x.w * x.r.profit : 0), 0) / wWins
        : null;
