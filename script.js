(() => {
  ...
  // Legacy helper: scroll to top then focus a specific element.
  // NOTE: We intentionally DO NOT call showPicker() here because it can pop the date/time picker
  // on desktop/mobile and feel intrusive. If you need the picker, let the user tap the field.
  function scrollToTopAndFocus(target, opts = {}) {
    const el = (typeof target === "string") ? $(target) : target;
    scrollToTop(opts);

    const delay = (opts && typeof opts.delayMs === "number") ? opts.delayMs : 60;
    setTimeout(() => {
      if (!el) return;
