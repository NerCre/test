...
symbol: $("#entry-symbol").value,
      ...
      notionUrl: ($("#entry-notionUrl")?.value ?? (base?.notionUrl ?? "")),
...
$("#entry-minWinRate").value = "30";
    ...
    { const el = $("#entry-notionUrl"); if (el) el.value = ""; }
...
function onJudge(shouldSave) {
  ...
  const base = wasEditing ? records.find((x) => x.id === editingEntryId) : null;
  const entry = getEntryForm(base);
  // normalize "その他"
...
$("#entry-price").value = r.entryPrice ?? "";
    ...
    { const el = $("#entry-notionUrl"); if (el) el.value = r.notionUrl || ""; }
