...
/** ---------------------------
 *  Storage backend (localStorage may be unavailable on some mobile browsers / private mode / in-app browsers)
 *  --------------------------*/
const storageBackend = (() => {
  try {
    const ls = window.localStorage;
    const k = "__tr_storage_test__";
    ls.setItem(k, "1");
    ls.removeItem(k);
    return { api: ls, persistent: true };
  } catch (e) {
    const mem = new Map();
    return {
      api: {
...
const storage = storageBackend.api;

let _storageNoticeShown = false;
function notifyStorageFallbackOnce() {
  if (_storageNoticeShown) return;
  if (storageBackend.persistent) return;
  _storageNoticeShown = true;
  try {
    showToast(
      "この環境ではローカル保存が無効です（プライベート/アプリ内ブラウザ等）。保存ボタンは動きますが、再読込でデータが消える可能性があります。",
      "error",
      4200
    );
  } catch (_) {
...
/**
 * Mobile (especially iOS) sometimes fails to fire click events reliably in certain contexts.
 * Bind both touchend and click, while preventing "ghost clicks".
 */
function bindTap(target, handler, opts = {}) {
  const el = (typeof target === "string") ? $(target) : target;
  if (!el) return;
  let lastTouchTs = 0;
  const touchWindowMs = (typeof opts.touchWindowMs === "number") ? opts.touchWindowMs : 800;

  const run = (ev) => {
    try {
      handler(ev);
    } catch (e) {
      console.error("tap handler failed:", e);
      try { showToast("処理中にエラーが発生しました。コンソールをご確認ください。", "error", 2400); } catch (_) {}
    }
  };
  ...
  el.addEventListener("click", (ev) => {
    if (Date.now() - lastTouchTs < touchWindowMs) return;
    run(ev);
  });
}
  ...
  function loadMasterSettings() {
    try {
      const raw = storage.getItem(MASTER_SETTINGS_KEY);
  ...
  try {
    storage.setItem(MASTER_SETTINGS_KEY, JSON.stringify(merged));
    notifyStorageFallbackOnce();
  } catch (e) {
    console.warn("Failed to save master settings:", e);
    // Do not break the app on mobile/private mode; just notify.
    try { showToast("判定マスタの保存に失敗しました（保存不可の環境か容量オーバーの可能性）。", "error", 2600); } catch (_) {}
  }
...
function setDefaultStatsRangeLast7Days() {
  ...
  function saveRecords(opts = {}) {
  const silent = !!opts.silent;

  // If persistent storage is unavailable, we still keep working (in-memory fallback).
  // Notify once so the user understands the limitation.
  notifyStorageFallbackOnce();
  ...
  try {
    ...
    console.warn("Failed to save to storage (possibly quota exceeded / unavailable)", e);
    if (!silent) {
      showToast(
        "保存に失敗しました（保存不可の環境/容量オーバーの可能性）。画像を削除するか、画像サイズを小さくしてから再度保存してください。",
        "error",
        2600
      );
    }
    // Do NOT throw: breaking the app on mobile is worse than a failed save.
    return false;
  }
}
...
function escapeHtml(s) {
   ...
   *  --------------------------*/
  function bind() {
        bindTap("#btn-judge", () => onJudge(false));
        bindTap("#btn-save-entry", () => onJudge(true));
...
$("#filter-indicatorKey").value = "";
      ...
      safeRenderStats();
        ...
        bindTap("#btnExportJson", exportJSON);
        bindTap("#btnImportJson", () => $("#import-file")?.click());
    $("#import-file")?.addEventListener("change", (e) => {
      ...
      e.target.value = "";
        ...
        bindTap("#btnExportCsv", () => exportCSV(getStatsFilteredRecords()));
}
...
function init() {
  ...
  records = loadRecords();
  // normalize/migrate on load. On mobile/private mode, storage may be unavailable; keep running.
  saveRecords({ silent: true });
  notifyStorageFallbackOnce();
