(() => {
  ...
  // Scroll to top only (no focus). Useful to avoid opening date/time pickers on mobile.
  function scrollToTopOnly(opts = {}) {
    const behavior = opts.behavior || "smooth";
    const blurActive = (opts.blurActive ?? true);
    if (blurActive) {
      try {
    ...
    elStatsTable.querySelectorAll("button[data-act]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (act === "edit-entry") {
          ...
          // NOTE: do not auto-focus datetime inputs (avoids opening pickers)
