...
out.entryPrice = out.entryPrice ?? null;
    ...
    out.completeness = out.completeness || "";
...
$("#entry-minWinRate").value = "30";
    ...
    // Notion URL field removed from UI (keep past data)
...
`;
  ...
  <label>完成度
    <select id="exit-completeness">
      <option value="">（未選択）</option>
      <option value="未完成" ${r.completeness === "未完成" ? "selected" : ""}>未完成</option>
      <option value="完全完成" ${r.completeness === "完全完成" ? "selected" : ""}>完全完成</option>
      <option value="未入力あり完成" ${r.completeness === "未入力あり完成" ? "selected" : ""}>未入力あり完成</option>
    </select>
  </label>
...
`;
  ...
  function updateExitProfitPreview(record) {
    ...
    const completeness = $("#exit-completeness")?.value || "";
...
$("#entry-price").value = r.entryPrice ?? "";
    ...
    const elNotion = $("#entry-notionUrl");
    if (elNotion) elNotion.value = r.notionUrl || "";
