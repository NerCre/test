(() => {
  ...
  const $ = (sel) => {
    ...
    const s = String(sel);
    // idを渡す呼び出し（例: $("entry-symbol")）と、CSSセレクタ呼び出し（例: $("#entry-symbol"), $(".tab")）の両対応
...
// Context Indicator (Entry) — multi-select stored as array (contextIndicators) and canonical string (market_context)
  const CONTEXT_INDICATOR_OPTIONS = [
    "なし",
    "Prep Signなし",
    "EMA Band GC維持",
    "EMA Band DC維持",
...
;
  ...
  function normalizeContextIndicators(v) {
    // Accept: array, comma-separated string, null/undefined
    let arr = [];
    if (Array.isArray(v)) {
      arr = v.map((x) => String(x || "").trim()).filter(Boolean);
    } else if (typeof v === "string") {
      arr = v.split(",").map((x) => x.trim()).filter(Boolean);
    } else if (v == null) {
      arr = [];
    } else {
    ...
    // keep only allowed
    arr = arr.filter((x) => CONTEXT_INDICATOR_OPTIONS.includes(x));

    // apply rule: "なし" is exclusive and default
    const others = arr.filter((x) => x !== "なし");
    if (others.length > 0) {
      const uniq = new Set(others);
      // preserve the defined option order (except "なし")
      return CONTEXT_INDICATOR_OPTIONS.filter((x) => x !== "なし" && uniq.has(x));
    }
    return ["なし"];
  }

  function contextIndicatorsToCanonical(arr) {
    const norm = normalizeContextIndicators(arr);
    if (norm.length === 1 && norm[0] === "なし") return "なし";
    return norm.join(", ");
  }

  function getEntryContextIndicators() {
    const nodes = document.querySelectorAll('input[name="entry-contextIndicator"]');
    const checked = [];
    nodes.forEach((n) => { if (n.checked) checked.push(String(n.value || "").trim()); });
    return normalizeContextIndicators(checked);
  }

  function setEntryContextIndicators(arr) {
    const norm = normalizeContextIndicators(arr);
    const nodes = document.querySelectorAll('input[name="entry-contextIndicator"]');
    nodes.forEach((n) => { n.checked = norm.includes(String(n.value || "").trim()); });
    updateEntryContextSummary();
  }

  function updateEntryContextSummary() {
    const el = $("#entry-contextSummary");
    if (!el) return;
    const norm = getEntryContextIndicators();
    el.textContent = (norm.length ? norm.join(", ") : "なし");
  }

  function initEntryContextIndicatorUI() {
    const nodes = document.querySelectorAll('input[name="entry-contextIndicator"]');
    if (!nodes || !nodes.length) return;

    const elNone = $("#entry-context-none");

    const applyRuleAndUpdate = (changed) => {
      const v = String(changed?.value || "").trim();
      if (v === "なし") {
        if (changed.checked) {
          nodes.forEach((n) => { if (n !== changed) n.checked = false; });
        } else {
          // do not allow all unchecked
          changed.checked = true;
        }
      } else {
        if (changed.checked && elNone) elNone.checked = false;

        const anyOther = Array.from(nodes).some((n) => (String(n.value||"").trim() !== "なし") && n.checked);
        if (!anyOther && elNone) elNone.checked = true;
      }
      updateEntryContextSummary();
    };

    nodes.forEach((n) => n.addEventListener("change", () => applyRuleAndUpdate(n)));
  ...
  // Scroll to top without focusing any input (avoids opening date/time pickers on mobile)
  function scrollToTopOnly(opts = {}) {
    const behavior = opts.behavior || "smooth";
    try { window.scrollTo({ top: 0, left: 0, behavior }); } catch (_) { window.scrollTo(0, 0); }
    // Some browsers keep scroll position on <html> / <body> separately; enforce as a fallback
...
/** ---------------------------
  ...
  const MASTER_DEFAULTS = Object.freeze({
    ...
    setupPriorN0: 30,          // Prior混合の基準N0（w=N0/(N0+N_setup))
        recencyHalfLifeDays: 90,  // 日付重み付け（半減期 日）
rrGateEnabled: true,     // RR gate strict（RRが下限未満なら原則ノーポジ）
    ...
    dataDataTypes: ["current"],
    dataCompletionStatuses: ["完全完成", "未入力あり完成"]});


  const MASTER_TRADE_TYPES = Object.freeze(["real", "virtual", "practice"]);
  const MASTER_TRADE_TYPES_DEFAULT = Object.freeze(["real", "virtual", "practice"]);
  const MASTER_DATA_TYPES = Object.freeze(["current", "legacy"]);
  const MASTER_DATA_TYPES_DEFAULT = Object.freeze(["current"]);
  ...
  function normalizeMasterTradeTypes(v) {
    ...
    const out = [];
    for (const x of arr) {
      const tt = normalizeTradeTypeValue(x);
      if (MASTER_TRADE_TYPES.includes(tt) && !out.includes(tt)) out.push(tt);
    }
    // Default uses current data only
    return out.length ? out : [...MASTER_TRADE_TYPES_DEFAULT];
  }

  
  function normalizeMasterDataTypes(v) {
    const arr = Array.isArray(v) ? v : [];
    const out = [];
    for (const x of arr) {
      const dt = normalizeDataCategoryValue(x);
      if (MASTER_DATA_TYPES.includes(dt) && !out.includes(dt)) out.push(dt);
    }
    return out.length ? out : [...MASTER_DATA_TYPES_DEFAULT];
  }
...
function normalizeMasterCompletionStatuses(v) {
  ...
  function loadMasterSettings() {
    try {
      ...
      // Data pool defaults: avoid mixing virtual/practice & incomplete data unless the user explicitly customized.
  ...
  function getMasterSettings() {
    ...
    _masterCache.dataDataTypes = normalizeMasterDataTypes(_masterCache.dataDataTypes);
  ...
  function initMasterTab() {
    ...
    const elRecencyHalfLifeDays = $("#master-recencyHalfLifeDays");

    const elUseReal = $("#master-use-real");
    const elUseVirtual = $("#master-use-virtual");
    const elUsePractice = $("#master-use-practice");
    const elUseDataCurrent = $("#master-use-data-current");
    const elUseDataLegacy = $("#master-use-data-legacy");
    ...
    if (elRecencyHalfLifeDays) elRecencyHalfLifeDays.value = String(s.recencyHalfLifeDays ?? MASTER_DEFAULTS.recencyHalfLifeDays);
    ...
    const dts = normalizeMasterDataTypes(s.dataDataTypes);
    if (elUseDataCurrent) elUseDataCurrent.checked = dts.includes("current");
    if (elUseDataLegacy) elUseDataLegacy.checked = dts.includes("legacy");
    ...
    $("#btn-master-save")?.addEventListener("click", () => {
      ...
      const dataDataTypes = [];
      if (elUseDataCurrent?.checked) dataDataTypes.push("current");
      if (elUseDataLegacy?.checked) dataDataTypes.push("legacy");
      ...
      if (!dataTradeTypes.length) {
        alert("判定に使用する取引種別が未選択です。少なくとも real / virtual / practice のいずれかをチェックしてください。");
        return;
      }
      if (!dataDataTypes.length) {
        alert("判定に使用するデータ区分が未選択です。少なくとも current / legacy のいずれかをチェックしてください。");
        return;
      }
      ...
      const next = {
        ...
        recencyHalfLifeDays: safeNum(elRecencyHalfLifeDays?.value),
        rrMin: Math.max(0, Number(elRrMin.value) || MASTER_DEFAULTS.rrMin),
        rrGateEnabled: !!elRrGate.checked,
        rrAllowMissing: !!elRrAllowMissing.checked,
        dataTradeTypes,
      ...
      // sanitize recencyHalfLifeDays
      if (next.recencyHalfLifeDays === null || next.recencyHalfLifeDays === undefined) {
        next.recencyHalfLifeDays = MASTER_DEFAULTS.recencyHalfLifeDays;
      }
      if (!Number.isFinite(next.recencyHalfLifeDays)) next.recencyHalfLifeDays = MASTER_DEFAULTS.recencyHalfLifeDays;
      next.recencyHalfLifeDays = Math.max(0, Math.trunc(next.recencyHalfLifeDays));
    ...
    $("#btn-master-reset")?.addEventListener("click", () => {
      ...
      if (elRecencyHalfLifeDays) elRecencyHalfLifeDays.value = String(d.recencyHalfLifeDays ?? MASTER_DEFAULTS.recencyHalfLifeDays);
      elRrMin.value = String(d.rrMin);
      elRrGate.checked = !!d.rrGateEnabled;
      elRrAllowMissing.checked = !!d.rrAllowMissing;
      elRrAllowMissing.disabled = false;
  ...
  const elExitFilterOpen = $("#exit-filter-open");
  const elExitFilterUnfinished = $("#exit-filter-unfinished");
  const elExitFilterAll = $("#exit-filter-all");


  // Exit filter mode (radio): open / unfinished / all
  function getExitFilterMode() {
    if (elExitFilterUnfinished && elExitFilterUnfinished.checked) return "unfinished";
    if (elExitFilterAll && elExitFilterAll.checked) return "all";
    return "open"; // default
  }

  function setExitFilterMode(mode = "open") {
    try {
      if (mode === "unfinished" && elExitFilterUnfinished) {
        elExitFilterUnfinished.checked = true;
      } else if (mode === "all" && elExitFilterAll) {
        elExitFilterAll.checked = true;
      } else if (elExitFilterOpen) {
        elExitFilterOpen.checked = true;
      }
    } catch (_) {}
  }
const elExitDetailsMain = $("#exit-details-main");
  ...
  // TP/SL必須切り替え（データ区分=legacy のときだけ必須解除 + 「必須」表示も連動）
  function setFieldRequiredAndBadge(selector, required) {
    const el = $(selector);
    if (!el) return;
    ...
    if (label) {
      const badge = label.querySelector(".req");
      if (badge) badge.hidden = !required;
    }
  }

  function applyDataCategoryTpSlRequirement() {
    const dataCategory = $("#entry-dataCategory")?.value || "current";
    const requireTpSl = (normalizeDataCategoryValue(dataCategory) !== "legacy");

    // TP/SL関連（Entry）
    ["#entry-LimitPrice", "#entry-tpType", "#entry-LossPrice", "#entry-lsType"].forEach((sel) => {
      setFieldRequiredAndBadge(sel, requireTpSl);
    });
  }
...
function setDefaultStatsRangeLast7Days() {
  ...
  function normalizeTradeTypeValue(v) {
    ...
    if (low === "real" || low === "virtual" || low === "practice" || low === "legacy") return low;
    return low;
  }

  function normalizeDataCategoryValue(v) {
    const raw = String(v ?? "").trim();
    if (!raw) return "";
    const map = {
      "CURRENT": "current",
    ...
    if (map[raw]) return map[raw];
    const low = raw.toLowerCase();
    if (low === "current" || low === "legacy") return low;
    return low;
  }


  // Date recency weight (recent trades have stronger influence).
  // halfLifeDays: 0 or less -> disabled (weight=1).
  function dateWeightForRecord(r, halfLifeDays) {
    const hl = Number(halfLifeDays);
    if (!Number.isFinite(hl) || hl <= 0) return 1;
    const dtStr = (r && (r.datetimeEntry || r.updatedAt || r.createdAt)) || "";
    const t = Date.parse(dtStr);
    if (!Number.isFinite(t)) return 0.5;
    const ageDays = Math.max(0, (Date.now() - t) / 86400000);
    const w = Math.exp(-Math.LN2 * ageDays / hl);
  ...
  function migrateRecord(r) {
    ...
    out.tradeType = normalizeTradeTypeValue(out.tradeType || "real");

    // Data category (current / legacy)
    out.dataCategory = normalizeDataCategoryValue(out.dataCategory || "");
    if (!out.dataCategory) {
      out.dataCategory = (out.tradeType === "legacy") ? "legacy" : "current";
    }
    // Backward compat: tradeType=legacy はデータ区分に寄せる（取引区分は real/virtual/practice に統一）
    if (out.tradeType === "legacy") {
      out.tradeType = "practice";
      out.dataCategory = "legacy";
    }
...
out.entryPrice = out.entryPrice ?? null;
    ...
    // Context Indicator (Entry) — backward compatible
    // Prefer explicit array, fallback to stored canonical string (market_context)
    out.contextIndicators = normalizeContextIndicators(out.contextIndicators ?? out.market_context ?? "");
    out.market_context = contextIndicatorsToCanonical(out.contextIndicators);
    ...
    out.ukiValue = out.ukiValue ?? null;
    out.nobiValue = out.nobiValue ?? null;
...
symbol: $("#entry-symbol").value,
      ...
      tradeType: normalizeTradeTypeValue($("#entry-tradeType").value),
      dataCategory: normalizeDataCategoryValue($("#entry-dataCategory")?.value || "current"),
      ...
      contextIndicators: getEntryContextIndicators(),
      market_context: contextIndicatorsToCanonical(getEntryContextIndicators()),
...
if (!entry.tradeType) return "取引区分は必須です。";
    ...
    if (!String(entry.decisiveIndicator || "").trim()) return "Trigger Indicator は必須です。";
    ...
    // legacy（過去データ）は「過去転記」用途のため、TP/SL（価格・種別）を必須にしない
    if (normalizeDataCategoryValue(entry.dataCategory || "current") !== "legacy") {
      ...
      if (entry.plannedLimitPrice === null) return "TP（利確価格）は必須です。";
      if (entry.cutLossPrice === null) return "SL（損切価格）は必須です。";
      // RRはTP/SLから自動計算されるため、計算不能は入力の整合性エラー
      if (entry.rr == null || !Number.isFinite(entry.rr) || entry.rr <= 0) return "RRが計算できません（Entry/TP/SL価格の整合性を確認）";
    } else {
      // legacy でも「その他」を選んだ場合は内容だけ整合させる
      if (entry.tpType && isTpOther(entry.tpType) && !String(entry.tpTypeOther || "").trim()) return "TP種別（その他）を入力してください。";
      if (entry.lsType && isLsOther(entry.lsType) && !String(entry.lsTypeOther || "").trim()) return "SL種別（その他）を入力してください。";
    }
  ...
  function clearEntryForm(opts = {}) {
    const doToast = !!opts.toast;
    editingEntryId = null;
    $("#entry-form").reset();
    ...
    $("#entry-dataCategory") && ($("#entry-dataCategory").value = "current");
    applyDataCategoryTpSlRequirement();
    $("#entry-direction").value = "";
...
$("#entry-minWinRate").value = "30";
    ...
    if (doToast) showToast("エントリーをクリアしました", "success");
    // UX: after clear, scroll top (no focus)
    scrollToTopOnly();
  }
...
function updateExitEditUI() {
  ...
  function renderJudge(result, symbol, timeframe) {
    ...
    const tt = (ms.dataTradeTypes || []).join(" / ") || "—";
    const dt = (ms.dataDataTypes || []).join(" / ") || "—";
    const cs = (ms.dataCompletionStatuses || []).join(" / ") || "—";
    const poolLine = `<p class="muted judge-meta">判定データ: <strong>${escapeHtml(tt)}</strong>（取引区分） / <strong>${escapeHtml(dt)}</strong>（データ区分） / <strong>${escapeHtml(cs)}</strong>（完成度）</p>`;
  ...
  function buildSetupStats(candidates, setupId, recHalfLifeDays) {
    const out = {};
    ...
    for (const d of dirs) {
      ...
      for (const r of rows) {
        const m = learnMetricR(r);
        if (m == null) continue;
        const w = dateWeightForRecord(r, recHalfLifeDays);
      ...
      if (wSum <= 0) continue;
      const ev = evSum / wSum;
      const winrate = (wWin / wSum) * 100;
      const avgProfit = wWin > 0 ? (profitSum / wWin) : null;
      const avgLoss = wLoss > 0 ? (lossSum / wLoss) : null;

      out[d] = { n, ev, winrate, avgProfit, avgLoss, wSum: round(wSum, 3) };
    }
    return out;
  }
...
function judge(current) {
    ...
    const allowDataTypes = normalizeMasterDataTypes(ms.dataDataTypes);
    const allowStatuses = normalizeMasterCompletionStatuses(ms.dataCompletionStatuses);

    // Stage A: trade type / data type / completion status
    const stageA = records.filter((r) => {
      const tt = normalizeTradeTypeValue(r.tradeType || "real");
        const dc = normalizeDataCategoryValue(r.dataCategory || "") || ((normalizeTradeTypeValue(r.tradeType || "") === "legacy") ? "legacy" : "current");
      const cs = normalizeCompletionStatusForJudge(r.completionStatus);
      return allowTypes.includes(tt) && allowDataTypes.includes(dc) && allowStatuses.includes(cs);
    });
debug.stages.afterTypeStatus = stageA.length;
    ...
    const REC_HL_DAYS = Number.isFinite(ms.recencyHalfLifeDays) ? ms.recencyHalfLifeDays : MASTER_DEFAULTS.recencyHalfLifeDays;

    debug.params = { TH, REC_HL_DAYS, MAX_NEIGHBORS, MIN_CASES, MIN_ESS, alpha, EV_MIN_R, EV_GAP_R, minComparableFeatures: (Number.isFinite(ms.minComparableFeatures) ? ms.minComparableFeatures : MASTER_DEFAULTS.minComparableFeatures) };
    ...
    const wSim = pseudoWS.map((x) => Math.pow(x.score, alpha));
    const wTime = pseudoWS.map((x) => dateWeightForRecord(x.r, REC_HL_DAYS));
    const w = wSim.map((ws, i) => ws * wTime[i]);
...
function onJudge(shouldSave) {
  ...
  if (err) {
    alert(err);
    // UX: after blocking alert, scroll top and focus entry datetime
    scrollToTopOnly();
    return;
  }
  ...
  const base = {
    ...
    // Context Indicator
    contextIndicators: entry.contextIndicators,
  ...
  clearEntryForm(); // resets Context Indicator / device default / judge output too
  updateExitEditUI();
}
  ...
  function updateExitSelect(targetId = null) {
    ...
    // Filter mode (radio): 未決済 / 未完成 / すべて
    let mode = getExitFilterMode();

    // Statsから targetId を指定して開く場合、フィルタで対象が消えるなら「すべて表示」に切替える
    if (targetId) {
      const t = records.find((x) => x.id === targetId);
      if (t) {
        const isVisible =
          (mode === "open" && !t.hasResult) ||
          (mode === "unfinished" && normalizeCompletionStatusForJudge(t.completionStatus) === "未完成") ||
          (mode === "all");
        if (!isVisible) {
          setExitFilterMode("all");
          mode = "all";
        }
      }
    }
    ...
    let list = sorted;
    if (mode === "open") {
      list = sorted.filter((r) => !r.hasResult);
    } else if (mode === "unfinished") {
      list = sorted.filter((r) => normalizeCompletionStatusForJudge(r.completionStatus) === "未完成");
    }
  ...
  function renderExitDetails(id) {
    ...
    const mAfter = computeAfterExitMetrics(
      dir,
      r.exitPrice,
      r.highDuringTrade,
      r.lowDuringTrade,
    ...
    const taeDisp  = (mAfter.taeValue  === null) ? "" : mAfter.taeValue;
    const ukiDisp  = (mAfter.ukiValue  === null) ? "" : mAfter.ukiValue;
    const nobiDisp = (mAfter.nobiValue === null) ? "" : mAfter.nobiValue;
...
`;
  ...
  <label id="uki-wrap" ${showTae ? "" : "hidden"}>浮き値
    <input id="uki-value" type="number" inputmode="decimal" step="0.1" value="${ukiDisp}" readonly>
  </label>

  <label id="nobi-wrap" ${showTae ? "" : "hidden"}>伸び値
    <input id="nobi-value" type="number" inputmode="decimal" step="0.1" value="${nobiDisp}" readonly>
  </label>

  <p id="tae-note" class="muted small" ${showTae ? "" : "hidden"}>
    ※ トレンド継続時のみ計算。勝ち(TP)=伸び値（＋押し目/戻りなしなら浮き値）、負け(SL)=耐え値＋伸び値
  </p>
...
`;
  ...
  // 決済後の相場が「トレンド継続」の場合に計算する値（耐え値/浮き値/伸び値）
  // - TP(勝ち): 伸び値（＋必要なら浮き値）
  // - SL(負け): 耐え値 + 伸び値
  // ※ 判定は profit の符号で行う（>0:勝ち / <0:負け / =0 or null:未計算）
  function computeAfterExitMetrics(dir, exitPrice, nH, nL, profit, postExitMarket) {
    const out = { taeValue: null, ukiValue: null, nobiValue: null };
    if (postExitMarket !== "trend_continue") return out;
    const ep = (typeof exitPrice === "number" && Number.isFinite(exitPrice)) ? exitPrice : null;
    if (ep === null) return out;
    const high = (typeof nH === "number" && Number.isFinite(nH)) ? nH : null; // nH
    const low  = (typeof nL === "number" && Number.isFinite(nL)) ? nL : null; // nL
    const p = (typeof profit === "number" && Number.isFinite(profit)) ? profit : null;
    const isWin  = (p !== null && p > 0);
    const isLose = (p !== null && p < 0);

    if (dir === "long") {
      if (isWin) {
        // TP（勝ち）
        if (low !== null && ep < low) out.ukiValue = (low - ep); // 押し目なく継続
        if (high !== null) out.nobiValue = (high - ep);
      } else if (isLose) {
        // SL（負け）
        if (low !== null) out.taeValue = (ep - low);
        if (high !== null) out.nobiValue = (high - ep);
      }
    } else if (dir === "short") {
      if (isWin) {
        // TP（勝ち）
        if (high !== null && ep > high) out.ukiValue = (ep - high); // 戻りなく継続
        if (low !== null) out.nobiValue = (ep - low);
      } else if (isLose) {
        // SL（負け）
        if (high !== null) out.taeValue = (high - ep);
        if (low !== null) out.nobiValue = (ep - low);
      }
    }
  ...
  function updateExitProfitPreview(record) {
    ...
    // 耐え値/浮き値/伸び値（条件: 決済後の相場=トレンド継続）
    const showTae = (postExitMarket === "trend_continue");
    const dir = (record.directionTaken || record.directionPlanned || "long");
    const mAfter = computeAfterExitMetrics(dir, exitPrice, high, low, profit, postExitMarket);
    const taeValue = mAfter.taeValue;
    const ukiValue = mAfter.ukiValue;
    const nobiValue = mAfter.nobiValue;

    const taeWrap = $("#tae-wrap");
    const ukiWrap = $("#uki-wrap");
    const nobiWrap = $("#nobi-wrap");
    const taeNote = $("#tae-note");
    const taeInput = $("#tae-value");
    const ukiInput = $("#uki-value");
    const nobiInput = $("#nobi-value");
    if (taeWrap) taeWrap.hidden = !showTae;
    if (ukiWrap) ukiWrap.hidden = !showTae;
    if (nobiWrap) nobiWrap.hidden = !showTae;
    if (taeNote) taeNote.hidden = !showTae;
    if (taeInput) taeInput.value = (taeValue === null ? "" : taeValue);
    if (ukiInput) ukiInput.value = (ukiValue === null ? "" : ukiValue);
    if (nobiInput) nobiInput.value = (nobiValue === null ? "" : nobiValue);
  ...
  function clearExitFormOnly() {
    ...
    const uw = $("#uki-wrap"); if (uw) uw.hidden = true;
    const nw = $("#nobi-wrap"); if (nw) nw.hidden = true;
  ...
  function clearExitSelectionAndForm() {
    ...
    setExitFilterMode("open");


    updateExitSelect();
    clearMsg();
    showToast("結果をクリアしました", "success");
    // UX: after clear, scroll top (no focus)
    scrollToTopOnly();
  }
  ...
  function saveExit() {
    ...
    if (!selectedExitId) {
      ...
      // UX: guide back to top + exit datetime
      scrollToTopOnly();
      return;
    }
    const r = records.find((x) => x.id === selectedExitId);
  ...
  function getFilters() {
    return {
      ...
      dataCategory: $("#filter-dataCategory")?.value || "",
...
function applyFilters(list, f) {
    return list.filter((r) => {
      ...
      if (f.tradeType && normalizeTradeTypeValue(r.tradeType) !== f.tradeType) return false;
      if (f.dataCategory) {
        const dc = normalizeDataCategoryValue(r.dataCategory || "") || ((normalizeTradeTypeValue(r.tradeType || "") === "legacy") ? "legacy" : "current");
        if (dc !== f.dataCategory) return false;
      }
...
function populateIndicatorStateFilterOptions(indicatorKey) {
  ...
  function loadRecordToEntry(id) {
    ...
    if ($("#entry-dataCategory")) $("#entry-dataCategory").value = normalizeDataCategoryValue(r.dataCategory || "") || ((normalizeTradeTypeValue(r.tradeType || "") === "legacy") ? "legacy" : "current");
...
$("#entry-price").value = r.entryPrice ?? "";
    ...
    // Context Indicator
    setEntryContextIndicators(r.contextIndicators ?? r.market_context ?? "");
...
function safeRenderStats() {
  ...
  function exportCSV(list = records) {
    ...
    const cols = [
      ...
      "datetimeEntry","entryWeekday","entrySession","symbol","timeframe","tradeType","dataCategory","directionPlanned",
      ...
      "hasResult","datetimeExit","exitPrice","highDuringTrade","lowDuringTrade","postExitMarket","completionStatus","taeValue","ukiValue","nobiValue","exitReason","exitReasonOther","profit",
...
function escapeHtml(s) {
  ...
  function bind() {
    ...
    $("#btn-clear-entry")?.addEventListener("click", () => clearEntryForm({ toast: true }));
...
elExitSelect?.addEventListener("change", () => {
    ...
    $("#exit-filter-open")?.addEventListener("change", () => updateExitSelect());
    $("#exit-filter-unfinished")?.addEventListener("change", () => updateExitSelect());
    $("#exit-filter-all")?.addEventListener("change", () => updateExitSelect());
$("#btn-exit-clear")?.addEventListener("click", () => clearExitSelectionAndForm());
    ...
    // 取引区分（Entry）
    $("#entry-tradeType")?.addEventListener("change", () => {
      updateEntryRRPreview();
    });

    // データ区分（Entry）: legacy のときは TP/SL を必須解除
    $("#entry-dataCategory")?.addEventListener("change", () => {
      applyDataCategoryTpSlRequirement();
      updateEntryRRPreview();
    });
// RR（Entry）: 価格が変わったら自動更新
    ...
    $("#btn-clear-filter")?.addEventListener("click", () => {
      ...
      $("#filter-dataCategory") && ($("#filter-dataCategory").value = "");
