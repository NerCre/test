...
rrGateEnabled: true,     // RR gate strict（RRが下限未満なら原則ノーポジ）
  ...
  const MASTER_TRADE_TYPES = Object.freeze(["real", "virtual", "practice", "legacy"]);
  ...
  function normalizeMasterTradeTypes(v) {
    ...
    // Default excludes legacy (legacy records are always excluded from Judge anyway)
  ...
  function initMasterTab() {
    ...
    const elUseLegacy = $("#master-use-legacy");
    ...
    if (elUseLegacy) elUseLegacy.checked = types.includes("legacy");
    ...
    $("#btn-master-save")?.addEventListener("click", () => {
      ...
      if (elUseLegacy?.checked) dataTradeTypes.push("legacy");
      ...
      if (!dataTradeTypes.filter((x) => x !== "legacy").length) {
        alert("判定に使用する取引種別が未選択です（legacyは常に判定対象外）。少なくとも real / virtual / practice のいずれかをチェックしてください。");
        return;
      }
    ...
    $("#btn-master-reset")?.addEventListener("click", () => {
      ...
      if (elUseLegacy) elUseLegacy.checked = t.includes("legacy");
...
const elExitDetailsMain = $("#exit-details-main");
  ...
  // TP/SL必須切り替え（取引区分=legacy のときだけ必須解除 + 「必須」表示も連動）
  ...
  function applyLegacyTpSlRequirement() {
    const tradeType = $("#entry-tradeType")?.value || "";
    const requireTpSl = (tradeType !== "legacy");
...
if (!entry.tradeType) return "取引区分は必須です。";
    ...
    // legacy は「過去転記」用途のため、TP/SL（価格・種別）を必須にしない
    if (entry.tradeType !== "legacy") {
...
function updateExitEditUI() {
  ...
  function renderJudge(result, symbol, timeframe) {
    ...
    const poolLine = `<p class="muted judge-meta">判定データ: <strong>${escapeHtml(tt)}</strong>（取引区分） / <strong>${escapeHtml(cs)}</strong>（完成度）</p>`;
...
function onJudge(shouldSave) {
  ...
  function renderExitDetails(id) {
    ...
    const dir = (r.directionTaken || r.directionPlanned || "long");
    const mPost = computePostExitTriplet(dir, r.profit, r.exitPrice, r.highDuringTrade, r.lowDuringTrade, r.postExitMarket);
    const showTae = mPost.show;
    const taeDisp = (mPost.taeValue === null) ? "" : mPost.taeValue;
    const ukiDisp = (mPost.ukiValue === null) ? "" : mPost.ukiValue;
    const nobiDisp = (mPost.nobiValue === null) ? "" : mPost.nobiValue;
...
`;
  ...
  <p id="tae-note" class="muted small" ${showTae ? "" : "hidden"}>
    ※ 勝ち（利益&gt;0）=TP扱い / 負け（利益&lt;0）=SL扱い（トレンド継続のみ自動計算）<br>
    ※ ロング: 伸び値=nH-exit /（押し目なし: exit&lt;nL）なら浮き値=nL-exit /（負け）耐え値=exit-nL<br>
    ※ ショート: 伸び値=exit-nL /（戻りなし: exit&gt;nH）なら浮き値=exit-nH /（負け）耐え値=nH-exit
  </p>
...
`;
  ...
  // 決済後の「耐え値/浮き値/伸び値」（条件: 決済後の相場=トレンド継続）
  // ルール（profitで勝ち/負けを判定。勝ち=TP扱い、負け=SL扱い）:
  // - TP勝ち:
  //   - ロング: 伸び値=nH-exit, （押し目なし: exit<nL）なら浮き値=nL-exit
  //   - ショート: 伸び値=exit-nL,（戻りなし: exit>nH）なら浮き値=exit-nH
  // - SL負け:
  //   - ロング: 耐え値=exit-nL, 伸び値=nH-exit
  //   - ショート: 耐え値=nH-exit, 伸び値=exit-nL
  function computePostExitTriplet(direction, profit, exitPrice, nH, nL, postExitMarket) {
    const show = (postExitMarket === "trend_continue");
    const okNum = (v) => (typeof v === "number" && Number.isFinite(v)) ? v : null;
    const dir = (direction === "short") ? "short" : "long";
    const _exit = okNum(exitPrice);
    const _nH = okNum(nH);
    const _nL = okNum(nL);
    const isWin = (typeof profit === "number" && Number.isFinite(profit) && profit > 0);
    const isLoss = (typeof profit === "number" && Number.isFinite(profit) && profit < 0);

    let taeValue = null;
    let ukiValue = null;
    let nobiValue = null;

    if (!show) return { show: false, taeValue, ukiValue, nobiValue };
    if (_exit === null) return { show: true, taeValue, ukiValue, nobiValue };

    if (isWin) {
      if (dir === "long") {
        if (_nH !== null) nobiValue = (_nH - _exit);
        if (_nL !== null && _exit < _nL) ukiValue = (_nL - _exit);
      } else {
        if (_nL !== null) nobiValue = (_exit - _nL);
        if (_nH !== null && _exit > _nH) ukiValue = (_exit - _nH);
      }
    } else if (isLoss) {
      if (dir === "long") {
        if (_nL !== null) taeValue = (_exit - _nL);
        if (_nH !== null) nobiValue = (_nH - _exit);
      } else {
        if (_nH !== null) taeValue = (_nH - _exit);
        if (_nL !== null) nobiValue = (_exit - _nL);
      }
    }

    return { show: true, taeValue, ukiValue, nobiValue };
  }
  ...
  function updateExitProfitPreview(record) {
    ...
    const mPost = computePostExitTriplet(dir, profit, exitPrice, high, low, postExitMarket);
    ...
    if (taeWrap) taeWrap.hidden = !mPost.show;
    if (ukiWrap) ukiWrap.hidden = !mPost.show;
    if (nobiWrap) nobiWrap.hidden = !mPost.show;
    if (taeNote) taeNote.hidden = !mPost.show;

    if (taeInput) taeInput.value = (mPost.taeValue === null ? "" : mPost.taeValue);
    if (ukiInput) ukiInput.value = (mPost.ukiValue === null ? "" : mPost.ukiValue);
    if (nobiInput) nobiInput.value = (mPost.nobiValue === null ? "" : mPost.nobiValue);

    const taeValue = mPost.taeValue;
    const ukiValue = mPost.ukiValue;
    const nobiValue = mPost.nobiValue;
...
$("#btn-exit-clear")?.addEventListener("click", () => clearExitSelectionAndForm());
    ...
    // 取引区分（Entry）: legacy のときは TP/SL を必須解除
