...
function onJudge(shouldSave) {
  ...
  const createdAt = editingEntryId ? (records.find((r) => r.id === id)?.createdAt || nowISO()) : nowISO();
	  ...
	  const oldRecForEntryMerge = records.find((r) => r.id === id);
  ...
  const base = {
    ...
    marketMemo: entry.marketMemo,
	    notionUrl: (document.getElementById("entry-notionUrl") ? entry.notionUrl : (oldRecForEntryMerge?.notionUrl || "")),


    // Entry image (optional). CSVには出力しないが、JSON/localStorage には保持する。
	    entryImageData: (document.getElementById("entry-image") ? entry.entryImageData : (oldRecForEntryMerge?.entryImageData || null)),
