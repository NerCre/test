...
function onJudge(shouldSave) {
  ...
  const prev = editingEntryId ? (records.find((r) => r.id === id) || null) : null;
  const createdAt = editingEntryId ? (prev?.createdAt || nowISO()) : nowISO();
  ...
  const base = {
    ...
    // Entry image (optional). 画面からは入力しない（過去データは保持）
    entryImageData: prev?.entryImageData ?? null,
