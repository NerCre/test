...
function completionMark(status) {
  ...
  if (status === "complete_with_missing") return "〇";
  return "—";
}
...
symbol: $("#entry-symbol").value,
      ...
      notionUrl: (() => { const el = $("#entry-notionUrl"); if (el) return el.value || ""; if (editingEntryId) return (records.find((r)=>r.id===editingEntryId)?.notionUrl || ""); return ""; })(),
...
$("#entry-minWinRate").value = "30";
    ...
    $("#entry-notionUrl") && ($("#entry-notionUrl").value = "");
...
<label>完成度
          <select id="exit-completion">
            <option value="">（未選択）</option>
            <option value="incomplete" ${r.completionStatus === "incomplete" ? "selected" : ""}>未完成</option>
            <option value="complete" ${r.completionStatus === "complete" ? "selected" : ""}>完全完成</option>
            <option value="complete_with_missing" ${r.completionStatus === "complete_with_missing" ? "selected" : ""}>未入力あり完成</option>
          </select>
        </label>
...
indicatorKey: $("#filter-indicatorKey")?.value || "",
      ...
      decisiveSignal: $("#filter-decisiveSignal")?.value || "",    completion: $("#filter-completion")?.value || "",
...
if (f.indicatorKey && f.indicatorState) {
      ...
      if (f.completion) {
        if ((r.completionStatus || "") !== f.completion) return false;
      }
  ...
  function renderStatsTable(list) {
    ...
    for (const r of sorted) {
      ...
      tr.innerHTML = `
        ...
        <td class="center">${completionMark(r.completionStatus)}</td>
        <td
...
$("#entry-price").value = r.entryPrice ?? "";
    ...
    $("#entry-notionUrl") && ($("#entry-notionUrl").value = r.notionUrl || "");
  ...
  function renderCharts(list) {
    ...
    // Chart 2: direction avg net profit
...
for (const d of dirs) {
  ...
  const n = group.length;
  avgNet2.push(n ? group.reduce((s, r) => s + r.profit, 0) / n : 0);
}
...
// Chart 3: timeframe win rate
    ...
    if (ctx2) {
      chartDirection = new Chart(ctx2, {
        ...
        data: {
          labels: labels2,
          datasets: [{ label: "平均損益(円)", data: avgNet2 }]
},
        options: { responsive: true, maintainAspectRatio: false }
      });
    }
...
$("#filter-indicatorKey").value = "";
      ...
      $("#filter-completion") && ($("#filter-completion").value = "");
