...
function getExitFilterMode() {
  const checked = document.querySelector('input[name="exit-filter"]:checked');
  const v = checked ? String(checked.value || "") : "";
  if (v === "unfinished" || v === "all" || v === "open") return v;
  return "open";
}

function setExitFilterMode(mode) {
  const m = mode === "unfinished" ? "unfinished" : (mode === "all" ? "all" : "open");
  if (m === "open" && elExitFilterOpen) elExitFilterOpen.checked = true;
  if (m === "unfinished" && elExitFilterUnfinished) elExitFilterUnfinished.checked = true;
  if (m === "all" && elExitFilterAll) elExitFilterAll.checked = true;
}
...
function onJudge(shouldSave) {
  ...
  const toastMsg = wasEditing ? "更新しました。" : "保存しました。";
  // After save, reset input + judge output + Context Indicator + device defaults
  clearEntryForm();
  ...
  function updateExitSelect(targetId = null) {
    ...
    // Filter mode: open / unfinished / all
    let mode = getExitFilterMode();

    // Statsから「結果編集」で特定IDを開きたいケースでは、
    // 現在のフィルタだとリストに出ないことがあるため、targetId 指定時は必要に応じて「全部」に切替える。
    if (targetId) {
      const t = records.find((x) => x.id === targetId);
      const isUnfinished = t ? (normalizeCompletionStatusForJudge(t.completionStatus) === "未完成") : false;
      if (t) {
        if (mode === "open" && t.hasResult) {
          setExitFilterMode("all");
          mode = "all";
        }
        if (mode === "unfinished" && !isUnfinished) {
          setExitFilterMode("all");
          mode = "all";
        }
      }
...
elExitSelect?.addEventListener("change", () => {
    ...
    // Exit: filter mode (未決済 / 未完成 / 全部)
    document.querySelectorAll('input[name="exit-filter"]').forEach((el) => {
      el.addEventListener("change", () => updateExitSelect());
    });
