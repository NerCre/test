(() => {
  ...
  const MASTER_DEFAULTS = Object.freeze({
    ...
    minCases: 6,             // MIN_CASES
    minEss: 3,               // MIN_ESS
    evMin: 0,                // EV_MIN
    evGap: 10,               // EV_GAP
    rrGateEnabled: true,     // RR gate strict
    rrMin: 1.0,              // RR_MIN
    rrAllowMissing: true     // allow when TP/SL missing
  });
...
function updateExitEditUI() {
  ...
  function isSameFeature(r, current) {
    ...
    function planMetrics(x) {
      const dir = x.directionPlanned || x.directionTaken || x.dir || x.direction || "";
    ...
    const sRR = relSim(curP.rr, recP.rr);
    if (sRR != null) { denom += wRR; match += wRR * sRR; }

    const sTP = relSim(curP.tpDist, recP.tpDist);
    if (sTP != null) { denom += wTPD; match += wTPD * sTP; }

    const sSL = relSim(curP.slDist, recP.slDist);
    if (sSL != null) { denom += wSLD; match += wSLD * sSL; }
...
// group by directionTaken
    ...
    const essBoost = clamp(Math.log10(ess + 1) / 1.2, 0, 1);
    const sampleBoost = clamp(countBoost * 0.4 + essBoost * 0.6, 0, 1);
    const confidence = clamp((baseWR * 0.7 + sampleBoost * 0.3) * 100, 0, 100);
    ...
    elStatsTable.querySelectorAll("button[data-act]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (act === "edit-entry") {
          ...
          updateExitSelect(id);
          gotoTab("exit");
          // iOS/Safari等で、タブ切替直後の scrollTo が効かない/戻ることがあるため
          // 次フレームでもう一度スクロールし、フォーカスも当てて確実に上へ。
          window.scrollTo({ top: 0, behavior: "smooth" });
          requestAnimationFrame(() => {
            window.scrollTo({ top: 0, behavior: "smooth" });
          });
          setTimeout(() => {
            window.scrollTo({ top: 0, behavior: "smooth" });
            // 先頭付近の要素にフォーカス（preventScrollで余計なスクロールは抑制）
            try { $("#exit-select")?.focus({ preventScroll: true }); } catch (_) {}
          }, 0);
        } else if (act === "delete") {
          deleteRecord(id);
        }
