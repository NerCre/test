(() => {
  ...
  const MASTER_DEFAULTS = Object.freeze({
    ...
    rrMin: 1.0,              // RR_MIN
    rrAllowMissing: false,    // TP/SL未入力（RR不明）はゲート対象外にする（ただし警告）
...
out.entryPrice = out.entryPrice ?? null;
  ...
  // Setup identity (decisive indicator × signal)
  out.setupId = out.setupId ?? makeSetupId(out.decisiveIndicator, out.decisiveSignal);
  out.setupIdInitial = out.setupIdInitial ?? out.setupId;
  out.setupTampered = Boolean(out.setupTampered);

  // Derived metrics (per-unit profit and R-multiple)
  out.profitPerUnit = out.profitPerUnit ?? null;
  out.rMultiple = out.rMultiple ?? null;
  if (out.hasResult) {
    const ppu = computeProfitPerUnit(out.symbol, out.directionTaken, out.entryPrice, out.exitPrice, out.feePerUnit);
    if (ppu != null) out.profitPerUnit = ppu;
    if (out.profit == null && ppu != null && typeof out.size === "number" && isFinite(out.size)) {
      out.profit = Math.round(ppu * out.size);
    }
    const rMult = computeRMultiple(out.symbol, out.directionTaken, out.entryPrice, out.exitPrice, out.cutLossPrice, out.feePerUnit);
    if (rMult != null) out.rMultiple = rMult;
  }
...
$("#entry-minWinRate").value = "30";
  ...
  // Lock decisive indicator after exit is saved (prevents after-the-fact edits)
  const di = $("#entry-decisiveIndicator");
  if (di) {
    const cur = records.find(r => r.id === editingEntryId);
    const locked = Boolean(cur && cur.hasResult);
    di.disabled = locked;
    di.title = locked ? "決済済みのため変更不可" : "";
  }
...
function updateExitEditUI() {
  ...
  function isSameFeature(r, current, ms) {
    ...
    // Setup prior (decisive indicator × signal): shrinkage toward setup-specific EV in R
    const setupKey = makeSetupId(current.decisiveIndicator, current.decisiveSignal);
    let setupCount = 0;
    let setupEv = null;
    if (setupKey) {
      const setupPool = records
        .map(migrateRecord)
        .filter(r => r.hasResult && !r.setupTampered && (r.setupIdInitial || r.setupId) === setupKey && r.directionTaken === d && r.rMultiple != null);
      setupCount = setupPool.length;
      if (setupCount > 0) {
        const sumR = setupPool.reduce((acc, r) => acc + Number(r.rMultiple || 0), 0);
        setupEv = sumR / setupCount;
      }
    }

    let evFinal = evKnn;
    let setupWeight = 1;
    if (setupEv != null && setupCount >= SETUP_PRIOR_MIN_N) {
      setupWeight = SETUP_PRIOR_N0 / (SETUP_PRIOR_N0 + setupCount);
      evFinal = (setupWeight * evKnn) + ((1 - setupWeight) * setupEv);
    }
...
// group by directionTaken (STRICT). Planned direction is not used for evaluation.
    ...
    for (const d of dirs) {
      ...
      // Weighted win prob
      const wWins = rows.reduce((s, x) => s + ((x.r.rMultiple > 0) ? x.w : 0), 0);
      const wLosses = rows.reduce((s, x) => s + ((x.r.profit < 0) ? x.w : 0), 0);

      const winRate = (wSum > 0) ? (wWins / wSum) * 100 : null;

      // Weighted avg profit of wins / losses (loss avg is negative)
      const avgProfit = (wWins > 0)
        ? rows.reduce((s, x) => s + ((x.r.rMultiple > 0) ? x.w * x.r.profit : 0), 0) / wWins
        : null;

      const avgLoss = (wLosses > 0)
        ? rows.reduce((s, x) => s + ((x.r.profit < 0) ? x.w * x.r.profit : 0), 0) / wLosses
        : null;

      // Expected value per trade (weighted mean PnL, yen)
      const ev = (wSum > 0)
        ? rows.reduce((s, x) => s + x.w * x.r.profit, 0) / wSum
        : null;
    ...
    // choose candidate direction based on expected value (yen/trade);
    ...
    if (recommendation === "flat") {
      ...
      if ((chosen.ev ?? -Infinity) < EV_MIN) {
        reason = `期待値(EV)=${(chosen.ev ?? 0).toFixed(1)}円 が下限 ${EV_MIN}円 未満。張る理由がない。`;
        recommendation = "flat";
        winRate = null;
      } else {
        const other = (recommendation === "long") ? (statsByDir.short || { n: 0 }) : (statsByDir.long || { n: 0 });
        if ((other.n ?? 0) > 0 && Math.abs((chosen.ev ?? 0) - (other.ev ?? 0)) < EV_GAP) {
          reason = `ロング/ショートの期待値差が小さい（差=${Math.abs((chosen.ev ?? 0) - (other.ev ?? 0)).toFixed(1)}円 < ${EV_GAP}円）。迷うなら張らない。`;
          recommendation = "flat";
          winRate = null;
        }
      }
...
function makeSetupId(decisiveIndicator, decisiveSignal) {
  const a = (decisiveIndicator || "").trim();
  const b = (decisiveSignal || "").trim();
  if (!a || !b) return "";
  return `${a}::${b}`;
}
...
function computeProfitPerUnit(symbol, direction, entryPrice, exitPrice, feePerUnit) {
  ...
  return computeProfit(symbol, direction, entryPrice, exitPrice, feePerUnit, 1);
}

function computeRiskPerUnit(symbol, entryPrice, cutLossPrice, feePerUnit) {
  if (entryPrice == null || cutLossPrice == null) return null;
  const mult = getMultiplierBySymbol(symbol);
  const diff = Math.abs(Number(entryPrice) - Number(cutLossPrice));
  if (!isFinite(diff) || diff <= 0) return null;
  const fee = Number(feePerUnit ?? 0);
  if (!isFinite(fee)) return null;
  const risk = diff * mult + fee;
  return (risk > 0 && isFinite(risk)) ? risk : null;
}

function computeRMultiple(symbol, direction, entryPrice, exitPrice, cutLossPrice, feePerUnit) {
  const ppu = computeProfitPerUnit(symbol, direction, entryPrice, exitPrice, feePerUnit);
  const risk = computeRiskPerUnit(symbol, entryPrice, cutLossPrice, feePerUnit);
  if (ppu == null || risk == null) return null;
  const r = ppu / risk;
...
function populateIndicatorStateFilterOptions(indicatorKey) {
  ...
  renderSetupStats(getFilteredRecords());
}
...
function safeRenderStats() {
  ...
  function exportCSV(list = records) {
    ...
    const cols = [
      ...
      "marketMemo","notionUrl","completionStatus","taeValue","postExitMarket","profitPerUnit","rMultiple","setupId","setupIdInitial","setupTampered","resultMemo"
    ];
...
function renderSetupStats(list) {
  const tbody = document.getElementById("setupStatsBody");
  if (!tbody) return;

  const rows = (list || [])
    .map(migrateRecord)
    .filter(r => r.hasResult && !r.setupTampered && r.rMultiple != null && (r.setupIdInitial || r.setupId));

  const groups = new Map();
  for (const r of rows) {
    const key = (r.setupIdInitial || r.setupId || "").trim();
    if (!key) continue;
  ...
  function maxDrawdownR(arr) {
    const a = [...arr].sort((x,y)=> (x.datetimeExit||x.datetimeEntry||"").localeCompare(y.datetimeExit||y.datetimeEntry||""));
    let equity = 0;
    let peak = 0;
    let maxDD = 0;
    for (const r of a) {
      equity += Number(r.rMultiple || 0);
  ...
  for (const [key, arr] of groups.entries()) {
    const n = arr.length;
    if (n < SETUP_STATS_MIN_N_DISPLAY) continue;
    const rs = arr.map(r => Number(r.rMultiple)).filter(x=>isFinite(x));
    if (!rs.length) continue;
    const wins = rs.filter(x=>x>0).length;
    const avgR = rs.reduce((a,x)=>a+x,0)/rs.length;
    const medR = median(rs);
    const grossP = rs.filter(x=>x>0).reduce((a,x)=>a+x,0);
    const grossL = rs.filter(x=>x<0).reduce((a,x)=>a+Math.abs(x),0);
    const pf = grossL === 0 ? (grossP>0 ? Infinity : null) : grossP/grossL;
    const mdd = maxDrawdownR(arr);

    const [ind, sig] = key.split("::");
    statsArr.push({ key, ind: ind || "", sig: sig || "", n, winRate: wins/rs.length, avgR, medR, pf, mdd });
  }

  statsArr.sort((a,b)=> (b.n - a.n) || (b.avgR - a.avgR));

  const fmtPct = v => (v*100).toFixed(1) + "%";
  const fmtR = v => (v==null || !isFinite(v)) ? "N/A" : (Math.round(v*100)/100).toFixed(2);
  const fmtPf = v => (v==null) ? "N/A" : (v===Infinity ? "∞" : (Math.round(v*100)/100).toFixed(2));

  tbody.innerHTML = statsArr.map(s => `
    <tr>
      <td>${escapeHtml(s.ind)}</td>
      <td>${escapeHtml(s.sig)}</td>
      <td>${s.n}</td>
  ...
  `).join("") || '<tr><td colspan="8" class="muted small">該当なし（N≥10のセットアップがありません）</td></tr>';
}
