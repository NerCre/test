...
symbol: $("#entry-symbol").value,
      ...
      entryImageData: (() => {
        if (!editingEntryId) return entryImageData;
        const cur = records.find((x) => x.id === editingEntryId);
        return (cur && cur.entryImageData !== undefined) ? cur.entryImageData : entryImageData;
      })()
    };
  }
...
$("#entry-minWinRate").value = "30";
    ...
    // Notion URLはUIから削除済み（過去データ保持のためレコード側は残す）
    const nu = $("#entry-notionUrl");
    if (nu) nu.value = "";
...
function updateExitEditUI() {
  ...
  const winRate = (result.winRate == null || !Number.isFinite(Number(result.winRate))) ? null : Number(result.winRate);
  const expected = (result.expectedMove == null || !Number.isFinite(Number(result.expectedMove)))
    ? "—"
    : `${Math.round(Number(result.expectedMove))}${escapeHtml(result.expectedMoveUnit || "円")}`;

  const minWin = Number.isFinite(result.minWinRate) ? result.minWinRate : 30;

  const recoClass = (result.recommendation === "flat"
    ? "reco-none"
    : (result.recommendation === "long" ? "reco-long" : (result.recommendation === "short" ? "reco-short" : "reco-none"))
  );

  const effN = (result.effectiveN == null || !Number.isFinite(Number(result.effectiveN))) ? null : Number(result.effectiveN);
  const effNText = (effN == null) ? "—" : `${Math.round(effN)}`;
  ...
  elJudgeOutput.innerHTML = `
    <div class="judge-grid">
      ...
      <div><strong>擬似ケース</strong><div>${result.pseudoCaseCount ?? 0}件</div></div>
      <div><strong>有効サンプル</strong><div>${effNText}</div></div>
    ...
    ${result.note ? `<p class="muted small">${escapeHtml(result.note)}</p>` : ``}
    ${result.recommendation !== "flat" && winRate != null && winRate < minWin ? `<p class="muted small">※ 勝率しきい値（${minWin}%）未満のため「ノーポジ推奨」扱いです。</p>` : ``}
  `;
}
  ...
  // Same symbol + timeframe, must have results
  ...
  // --- Similarity/weighting settings (tune here)
  const SCORE_FLOOR = 0.55;      // include "somewhat similar" instead of only perfect matches
  const TOP_K = 60;              // use top-K neighbors
  const SIM_POWER = 2.0;         // emphasize high similarity (1.0=linear, 2.0=stronger)
  const HALF_LIFE_DAYS = 60;     // time-decay: half weight every N days
  const MIN_EFFECTIVE_N = 10;    // guard against "たまたま勝った"
  const DECISIVE_BOOST = 1.15;   // boost when decisiveIndicator matches
  const DECISIVE_SIGNAL_BOOST = 1.10; // boost when decisiveSignal matches (optional)
  ...
  const timeDecay = (r) => {
    const t = dtToMs(r.datetimeEntry) ?? dtToMs(r.datetimeExit) ?? null;
    if (!t) return 1;
  ...
  if (!candidates.length) {
    return {
      ...
      minWinRate,
      note: "同一銘柄・同一時間足の実績データがまだありません。"
    };
  }

  // Build weighted neighbors (kNN-style)
  const neighbors = candidates
    .map((r) => {
      const score = isSameFeature(r, current); // 0..1
      if (!(score > 0)) return null;

      // decisive indicator bonus (only when the user selected it)
      let bonus = 1;
      if (current.decisiveIndicator && r.decisiveIndicator && current.decisiveIndicator === r.decisiveIndicator) {
        bonus *= DECISIVE_BOOST;
        if (current.decisiveSignal && r.decisiveSignal && current.decisiveSignal === r.decisiveSignal) {
          bonus *= DECISIVE_SIGNAL_BOOST;
        }
      }

      const decay = timeDecay(r);

      // weight: similarity^p * (decisive bonus) * time decay
      const weight = Math.pow(score, SIM_POWER) * bonus * decay;

      return { r, score, weight, bonus, decay };
    })
    .filter(Boolean)
  ...
  const pseudoCaseCount = neighbors.length;
  ...
  if (!pseudoCaseCount) {
    return {
      ...
      scoreFloor: SCORE_FLOOR,
      minWinRate,
      note: "類似度が高い過去データが見つかりませんでした（条件が厳しすぎる可能性があります）。"
    };
  }

  const sum = (arr, fn) => arr.reduce((s, x) => s + (fn ? fn(x) : x), 0);

  const wSum = sum(neighbors, (x) => x.weight);
  const w2Sum = sum(neighbors, (x) => x.weight * x.weight);
  const effectiveN = (w2Sum > 0) ? (wSum * wSum) / w2Sum : 0;

  const byDir = { long: [], short: [] };
  for (const n of neighbors) {
    const d = (n.r.directionTaken || n.r.directionPlanned || "long");
    if (d === "long" || d === "short") byDir[d].push(n);
  }

  const calcDirStats = (dir) => {
    const list = byDir[dir] || [];
    const wAll = sum(list, (x) => x.weight);
    if (!(wAll > 0)) {
      return { n: 0, wAll: 0, winRate: null, avgProfit: null, avgLoss: null, expectedMove: null, meanProfit: null };
    }

    const wins = list.filter((x) => x.r.profit > 0);
    const losses = list.filter((x) => x.r.profit < 0);

    const wWins = sum(wins, (x) => x.weight);
    const wLoss = sum(losses, (x) => x.weight);

    const winRate = (wAll > 0) ? (wWins / wAll) * 100 : null;

    const avgProfit = (wWins > 0)
      ? sum(wins, (x) => x.weight * x.r.profit) / wWins
      : null;

    const avgLoss = (wLoss > 0)
      ? sum(losses, (x) => x.weight * x.r.profit) / wLoss
      : null;

    // overall mean profit (expected value in yen)
    const meanProfit = sum(list, (x) => x.weight * x.r.profit) / wAll;

    // expectedMove: price based (no multiplier)
    let expectedMove = null;
    if (dir === "long") {
      const moves = list
        .map((x) => (typeof x.r.highDuringTrade === "number" && typeof x.r.entryPrice === "number")
          ? Math.max(0, x.r.highDuringTrade - x.r.entryPrice)
          : null
        )
        .filter((v) => typeof v === "number" && Number.isFinite(v));
      expectedMove = moves.length
        ? sum(list, (x) => {
            if (!(typeof x.r.highDuringTrade === "number" && typeof x.r.entryPrice === "number")) return 0;
            return x.weight * Math.max(0, x.r.highDuringTrade - x.r.entryPrice);
          }) / wAll
        : null;
    } else if (dir === "short") {
      const moves = list
        .map((x) => (typeof x.r.lowDuringTrade === "number" && typeof x.r.entryPrice === "number")
          ? Math.max(0, x.r.entryPrice - x.r.lowDuringTrade)
          : null
        )
        .filter((v) => typeof v === "number" && Number.isFinite(v));
      expectedMove = moves.length
        ? sum(list, (x) => {
            if (!(typeof x.r.lowDuringTrade === "number" && typeof x.r.entryPrice === "number")) return 0;
            return x.weight * Math.max(0, x.r.entryPrice - x.r.lowDuringTrade);
          }) / wAll
        : null;
    }

    return { n: list.length, wAll, winRate, avgProfit, avgLoss, expectedMove, meanProfit };
  };

  const statsLong = calcDirStats("long");
  const statsShort = calcDirStats("short");

  // choose candidate direction based on weighted expected value; tie-break by winRate then weight
  let candidate = "flat";
  const choices = [
    { dir: "long", s: statsLong },
    { dir: "short", s: statsShort }
  ].filter((x) => x.s.n > 0);

  if (choices.length) {
    choices.sort((a, b) => {
      const A = a.s, B = b.s;
      if ((B.meanProfit ?? -Infinity) !== (A.meanProfit ?? -Infinity)) return (B.meanProfit ?? -Infinity) - (A.meanProfit ?? -Infinity);
      if ((B.winRate ?? -Infinity) !== (A.winRate ?? -Infinity)) return (B.winRate ?? -Infinity) - (A.winRate ?? -Infinity);
      return (B.wAll ?? 0) - (A.wAll ?? 0);
    });
    candidate = choices[0].dir;
  }

  const chosen = (candidate === "long") ? statsLong : (candidate === "short" ? statsShort : {});

  // Base recommendation
  let recommendation = candidate;
  let winRate = chosen.winRate ?? null;

  // Guard #1: minimum effective sample size
  let note = "";
  if (effectiveN < MIN_EFFECTIVE_N) {
    recommendation = "flat";
    note = `有効サンプルが不足しています（有効N=${Math.round(effectiveN)}、目安=${MIN_EFFECTIVE_N}）。「たまたま勝ち」を誤判定しないためノーポジ推奨にしています。`;
  }

  // Guard #2: minimum win-rate threshold
  if (recommendation !== "flat" && winRate != null && winRate < minWinRate) {
    recommendation = "flat";
    note = note || `勝率しきい値（${minWinRate}%）未満のためノーポジ推奨にしています。`;
  }

  // Decisive indicator "prior" bonus (separate from similarity bonus): if the chosen decisive indicator itself tends to win, confidence up a bit.
  let decisiveBonusPoints = 0;
  if (current.decisiveIndicator && (candidate === "long" || candidate === "short")) {
    const sameKey = candidates.filter((r) =>
      r.decisiveIndicator === current.decisiveIndicator &&
      (r.directionTaken || r.directionPlanned || "long") === candidate &&
      typeof r.profit === "number" && Number.isFinite(r.profit)
    );
    if (sameKey.length >= 10) {
      const dW = (r) => timeDecay(r);
      const dwAll = sameKey.reduce((s, r) => s + dW(r), 0);
      const dwWins = sameKey.reduce((s, r) => s + (r.profit > 0 ? dW(r) : 0), 0);
      const decisiveWR = (dwAll > 0) ? (dwWins / dwAll) * 100 : null;
      if (decisiveWR != null && winRate != null) {
        const diff = decisiveWR - winRate;
        if (diff > 3) decisiveBonusPoints = clamp(diff / 30, 0, 1) * 8; // max +8pt
      }
    }
  }

  // confidence: winRate + effectiveN, then apply decisive bonus (but keep 0..100)
  const baseWR = (winRate == null ? 0 : winRate) / 100; // 0..1
  const nBoost = clamp(Math.log10(effectiveN + 1) / 1.2, 0, 1); // 0..1
  let confidence = clamp((baseWR * 0.7 + nBoost * 0.3) * 100 + decisiveBonusPoints, 0, 100);

  // If recommendation was forced to flat due to guards, lower confidence to avoid "強い断言"
  if (recommendation === "flat") {
    confidence = clamp(confidence * 0.6, 0, 100);
    winRate = null;
  }
...
function onJudge(shouldSave) {
  ...
  function updateExitSelect() {
    ...
    const onlyOpen = !!(elExitOnlyOpen && elExitOnlyOpen.checked);
